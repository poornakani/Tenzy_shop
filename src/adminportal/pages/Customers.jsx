...
import { CUSTOMERS } from "../dummydata/index";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Active", value: CUSTOMERS.filter((c) => c.status === "active").length, icon: Users, color: "bg-emerald-500" },
          ...
          <div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-slate-50 rounded-xl py-2">
                <p className="text-xs font-bold text-slate-800">{c.orders}</p>
                <p className="text-[10px] text-slate-400">Orders</p>
              </div>
              <div className="bg-slate-50 rounded-xl py-2">
                <p className="text-xs font-bold text-slate-800">LKR {fmt(c.totalSpent)}</p>
          ...
          <tbody className="divide-y divide-slate-50">
            ...
            {filtered.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-5 py-3 text-xs text-slate-500">{c.joined}</td>
                <td className="px-5 py-3 text-xs font-bold text-slate-800 text-center">{c.orders}</td>
                <td className="px-5 py-3 text-xs font-bold text-slate-800">LKR {fmt(c.totalSpent)}</td>
                <td className="px-5 py-3 text-xs text-slate-500">{c.lastOrder}</td>
                <td className="px-5 py-3">
